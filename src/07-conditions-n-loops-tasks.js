...
function isInsideCircle(circle, point) {
  const katX = point.x - circle.center.x;
  const katY = point.y - circle.center.y;
  const gip = Math.sqrt(katX ** 2 + katY ** 2);
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const double = arr.filter((elem) => elem === arr[i]);
    if (double.length < 2) {
      return arr[i];
    }
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const str = [a, b].sort((x, y) => x - y).join(', ');
...
function isCreditCardNumber(ccn) {
  const result = [...String(ccn)].reverse().map((elem) => Number(elem)).map((elem, index) => {
    if ((index + 1) % 2 === 0) {
      if (elem * 2 > 9) {
        return (elem * 2) - 9;
      }
...
function getDigitalRoot(num) {
  const sum = [...String(num)].map((elem) => Number(elem)).reduce((sumVal, curr) => sumVal + curr);
  if (sum > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < [...str].length; i += 1) {
    ...
    if (closeBrackets.includes([...str][i]) && stack.length === 0) {
      return false;
    }
    if (closeBrackets.includes([...str][i]) && stack.length !== 0) {
...
function getCommonDirectoryPath(pathes) {
  ...
  const checkDuplicate = () => {
    const dublicate = pathes.filter((element) => element.slice(0, savePath)
    === pathes[0].slice(0, savePath));
    if (dublicate.length === pathes.length) {
      savePath += 1;
      return checkDuplicate();
    }
    if (dublicate.length !== pathes.length && savePath === 1) {
      return '';
    }
    if (dublicate.length !== pathes.length) {
      const lastSlash = [...pathes[0].slice(0, savePath - 1)].reverse().indexOf('/');
      return [...pathes[0].slice(0, savePath - 1)].reverse().slice(lastSlash).reverse().join('');
    }
    return '';
  };
  return checkDuplicate();
...
function getMatrixProduct(m1, m2) {
  ...
  m1.forEach((row, indexRow) => {
    result.push([]);
    row.forEach((e, indexIteration) => {
      let count = 0;
      row.forEach((elem, index) => {
        count += (elem * m2[index][indexIteration]);
      });
      if (count) {
        result[indexRow].push(count);
      }
    });
  });
  return result;
