...
function timeSpanToString(startDate, endDate) {
  ...
  let minutes = parseInt(duration / (1000 * 60), 10) % 60;
  let hours = parseInt(duration / (1000 * 60 * 60), 10) % 24;
...
function angleBetweenClockHands(date) {
  ...
  let deg = Math.abs((hour + min / 60) * 30 - min * 6);
  if (deg > 180) {
    deg = 360 - deg;
  }
  const result = Math.abs(deg * Math.PI) / 180;
  return result;
}
